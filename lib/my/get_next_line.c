...
char *my_strcat_with_endline(char *dest, char *src)
{
	int i = 0;
	int len_d = 0;
	int len_s = 0;
	...
	for (; dest && dest[len_d] != 0 && dest[len_d] != '\n'; len_d++);
	for (; src && src[len_s] != 0 && src[len_s] != '\n'; len_s++);
...
char *loop_until_endline(int fd, char *buffer, char *result, int *endline)
{
	...
	while ((a = read(fd, buffer, 1)) > 0) {
		buffer[a] = 0;
		result = my_strcat_with_endline(result, buffer);
...
char *get_next_line(int fd)
{
	...
	result = (buffer != NULL ? my_strcat_with_endline(result, buffer) : 0);
